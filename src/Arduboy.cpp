...
{
  int end = x+w;
  for (int a = max(0,(int)x); a < min(end,WIDTH); a++)
  {
    drawPixel(a,y,color);
  }
}
...
{
  u_char c = (color ? 0xff:0);
  for (int i = 0; i < sizeof(sBuffer); i++){*(sBuffer + i) = c;}
}
...
boolean Arduboy::notPressed(uint8_t buttons)
{
  ...
  return (getInput() & buttons ? false:true);
}
